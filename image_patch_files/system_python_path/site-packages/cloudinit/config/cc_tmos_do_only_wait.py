...
wait unti f5-declarative-onboarding completes.
"""
...
LOG = logging.getLogger('tmos_do_only_wait')
...
def runs_post_onboard_commands(name):
    ...
    flag_file = "%s%s%s" % (
        POST_ONBOARD_FLAG_FILE_DIR_PREFIX,
        name,
...
def clean():
    ...
    injected_already = subprocess.Popen(
        "cat /config/startup | grep " + DO_DELAYED_DEPLOYMENT_SCRIPT + " | wc -l",
        stdout=subprocess.PIPE, shell=True
    ).communicate()[0].replace('\n', '')
    if injected_already == '1':
        subprocess.call(['/bin/sed', '-i', "/%s/d" %
                         DO_DELAYED_DEPLOYMENT_SCRIPT, '/config/startup'])
...
def handle(name, cloud_config, cloud, log, args):
    ...
    try:
        ...
        LOG.error(
            'onboard exception prior to f5-declarative-onboard declaration - %s', err)
        onboard_status = ERROR
    ...
    if os.path.exists(CLOUD_CONFIG_FILE):
        ...
        if 'phone_home_url_verify_tls' in cloud_config[name]:
            phone_home_url_verify_tls = cloud_config[name]['phone_home_url_verify_tls']
