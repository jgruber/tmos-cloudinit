...
#
"""
This module reads metadata from DHCP requests made on TMM interfaces.
...
# constants
OUT_DIR = '/config/cloud/tmos_dhcp_tmm'
...
def resolve_resources(rd_enabled=True, default_route_interface=None,
    ...
    # resolve data from mgmt interface DHCP lease
    try:
    ...
    for interface in get_linux_interfaces():
        if interface:
            int_index = interface[3:]
            tmm_interface_name = '1.' + int_index
                    ...
                    if s_ip:
                        ...
                        if s_gw and s_gw != m_gw:
                            ...
                            if default_route_interface == links[tmm_interface_name]['interface']:
                                log_progress(
                                    'default_route_interface discovered with DHCP as interface: %s gw: %s' % (links[tmm_interface_name]['interface'], s_gw))
                                route['use_rd'] = False
                                number_of_default_routes += 1
                            if configsync_interface == links[tmm_interface_name]['interface']:
                                log_progress(
                                    'configsync_interface discovered with DHCP as link: ' + tmm_interface_name)
                                route['use_rd'] = False
                            ...
                            if (not default_route_interface) and (number_of_default_routes == 0 and inject_route):
                                log_progress(
                                    'dynamically setting default_route_interface with DHCP interface: %s gw: %s' % (links[tmm_interface_name]['interface'], s_gw))
                                route['use_rd'] = False
    ...
    if 'hostname' not in services:
        log_progress('forcing device hostname to: bigip')
