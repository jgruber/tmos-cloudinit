...
# constants
OUT_DIR = '/config/cloud/openstack'

PRE_DO_TMSH_CMD_FILE_DIR = OUT_DIR + '/initscripts/all'
NON_DO_TMSH_CMD_FILE_DIR = OUT_DIR + '/initscripts/non_declared'
POST_ONBOARD_CMD_FILE_DIR = OUT_DIR + '/initscripts/post'
...
# we ensure our own directories because the built in util
# uses SELinuxGuard, which we don't know will always be
...
def copy_meta_data(source_dir):
    ...
    log_progress('copying metadata file')
...
def inject_public_ssh_keys(log, keys):
    ...
    log_progress('injecting root SSH keys')
    if not keys:
        keys = []
    if os.path.isfile(META_DATA_FILE):
        ...
        util.logexc(
            log, "could not find OpenStack meta_data to inject SSH keys")
    try:
        tmos_onboard_utils.inject_public_ssh_keys(keys)
    except Exception as err:
        log_progress('exception in injecting SSH keys: %s' % err)


# copies OpenStack meta_data.json from config drive
def copy_network_data(source_dir):
    """Copies the OpenStack network_data.json file into the Metadata directory"""
    log_progress('copying network metadata file')
    fpath = os.path.join(source_dir, "openstack",
                         "latest", "network_data.json")
    if os.path.isfile(fpath):
        metadata_dir_exists()
...
def get_metadata():
    ...
    log_progress('parsing metadata files')
    os_nmd, os_md = {}, {}
...
def initialize_do_declaration():
    ...
    log_progress('intializing f5-declarative-onboarding declaration')
    return {
        'schemaVersion': "1.0.0",
        'class': 'Device',
        'async': True,
...
def resolve_resources(rd_enabled=True, default_route_interface=None,
                      configsync_interface=None, inject_routes=True,
                      do_enabled=False, dhcp_timeout=120):
    """Resolve the resource provisioning dataset from metadata"""
    (os_nmd, os_md) = get_metadata()
    if not (bool(os_nmd) or bool(os_md)):
        log_progress('OpenStack metadata unavailable.. halting processing')
        return ({}, {}, {}, {})
    do_declaration = None
    if do_enabled:
        do_declaration = initialize_do_declaration()
    if default_route_interface:
        log_progress('default_route_interface ' +
                     default_route_interface + ' requested')
    else:
        log_progress(
    ...
    log_progress('setting configsync_interface to ' + configsync_interface)
    ...
    for link in os_nmd['links']:
        ...
        if link['type']in ln_types:
            ...
            links[link['id']] = {'net_name': net_name, 'mtu': link['mtu'], 'interface': '1.%s' % n_idx, 'route_domain': 0,
                                 'interface_index': n_idx, 'segmentation_id': 4094-n_idx, 'tagged': False}
            n_idx += 1
    # resolve L2 virtual networks
    for link in os_nmd['links']:
        if link['type'] == 'vlan':
            if link['vlan_link'] not in links:
                log_progress("VLAN %s defined for unsupported link %s" % (
                ...
                if not onenic and links[link['vlan_link']]['interface_index'] == 0:
                    ...
                    links[link['id']] = {'net_name': 'vlan_%s' % link['vlan_id'], 'mtu': links[link['vlan_link']]['mtu'], 'interface': links[link['vlan_link']]['interface'],
                                         'interface_index': links[link['vlan_link']]['interface_index'], 'segmentation_id': link['vlan_id'], 'tagged': True}
    for net in os_nmd['networks']:
        # resolve DHCP discovered attributes
        if net['type'] == 'ipv4_dhcp' or net['type'] == 'ipv6_dhcp':
            if net['link'] == m_l_id:
                try:
                    mgmt_data = tmos_onboard_utils.process_dhcp_lease(
                    ...
                    log_progress(
                        'exception in processing mgmt DHCP lease file: %s' % err)
            else:
                ...
                try:
                    if tmos_onboard_utils.make_dhcp_request(interface, dhcp_timeout):
                        interface_data = tmos_onboard_utils.process_dhcp_lease(
                            interface)
                        ...
                        if s_gw and s_gw != m_gw:
                            ...
                            route['use_rd'] = True
                            route['exclude'] = False
                            if default_route_interface == links[net['link']]['interface']:
                                log_progress(
                                    'default_route_interface discovered with DHCP as interface: %s gw: %s' % (links[net['link']]['interface'], s_gw))
                                route['use_rd'] = False
                                number_of_default_routes += 1
                            if configsync_interface == links[net['link']]['interface']:
                                log_progress(
                                    'configsync_interface discovered with DHCP as link: ' + net['link'])
                                route['use_rd'] = False
                            ...
                            if (not default_route_interface) and (number_of_default_routes == 0 and inject_route):
                                log_progress(
                                    'dynamically setting default_route_interface with DHCP interface: %s gw: %s' % (links[net['link']]['interface'], s_gw))
                                route['use_rd'] = False
                        ...
                        for route in tmos_onboard_utils.process_dhcp_routes(s_routes):
                            route['use_rd'] = True
                            route['exclude'] = False
                            if default_route_interface == links[net['link']]['interface']:
                                route['use_rd'] = False
                        ...
                        selfips[net['link']] = {'selfip_name': selfip_name, 'net_name': links[net['link']]['net_name'],
                    ...
                    log_progress(
                        'exception processing DHCP for interface %s - %s' % (interface, err))
        else:
            if net['link'] == m_l_id:
                m_ip = net['ip_address']
                m_nm = net['netmask']
                ...
                if net['type'] in n_types:
                        ...
                        if net['link'] in links:
                            ...
                            if route['network'] == '0.0.0.0' or route['network'] == '::':
                                if route['gateway'] != m_gw:
                                    ...
                                    if default_route_interface == links[net['link']]['interface']:
                                        log_progress(
                                            'default_route_interface discovered as interface: %s gw: %s' % (links[net['link']]['interface'], route['gateway']))
                                        route['use_rd'] = False
                                        number_of_default_routes += 1
                                    if configsync_interface == links[net['link']]['interface']:
                                        log_progress(
                                            'configsync_interface discovered as interface: ' + links[net['link']]['interface'])
                                        route['use_rd'] = False
                                        if not default_route_interface == configsync_interface:
                                            inject_route = False
                                    if (not default_route_interface) and (number_of_default_routes == 0 and inject_route):
                                        log_progress(
                                            'dynamically setting default_route_interface as interface: %s gw: %s' % (links[net['link']]['interface'], route['gateway']))
                                        route['use_rd'] = False
                                        number_of_default_routes += 1
                    ...
                    if net['link'] in links:
                        ...
                        selfip_name = selfip_name.replace('-', '_')
                        selfips[net['link']] = {'selfip_name': selfip_name, 'net_name': links[net['link']]['net_name'],
                                                'ip_address': net['ip_address'], 'netmask': net['netmask']}
    if rd_enabled:
        log_progress('decorating route domains')
            ...
            for route in routes[n_link]:
                if route['use_rd']:
                    ...
                    if selfips[n_link]['ip_address'].find('%') < 0:
                        selfips[n_link]['ip_address'] = selfips[n_link]['ip_address'] + '%' + r_dom
        ...
        for n_link in routes:
            for route in routes[n_link]:
                if route['use_rd'] and (route['network'] == '0.0.0.0' or route['network'] == '::'):
                    log_progress(
                        'excluding default route on link %s because route domains disabled' % n_link)
                    route['exclude'] = True

    log_progress('resolving service resources')
    dns_services = []
    ntp_services = []
    if 'hostname' not in services:
        log_progress('forcing device hostname to: ' + os_md['name'])
        services['hostname'] = os_md['name']
    if 'domainname' not in services:
        log_progress('forcing device domainname to .local')
        services['domainname'] = 'local'
    for service in os_nmd['services']:
        if service['type'] == 'dns' and (tmos_onboard_utils.is_v6(service['address']) or tmos_onboard_utils.is_v4(service['address'])):
            log_progress('discovered DNS service configuration')
            dns_services.append(service['address'])
        if service['type'] == 'ntp' and (tmos_onboard_utils.is_v6(service['address']) or tmos_onboard_utils.is_v4(service['address'])):
            log_progress('discovered NTP service configuration')
    ...
    if onenic and do_enabled:
        do_declaration['Common']['internal'] = {
            ...
            'mtu': m_l_mtu,
            'tag': 4094,
            'interfaces': [{'name': '1.0', 'tagged': False}]
        }
        if len(m_nm) > 3:
            if tmos_onboard_utils.is_v4(m_ip):
                m_nm = tmos_onboard_utils.ipv4_cidr_from_netmask(m_nm)
            else:
                m_nm = tmos_onboard_utils.ipv6_cidr_from_netmask(m_nm)
    ...
    if do_enabled:
        ...
        for l_id in links:
            ...
            do_declaration['Common'][link['net_name']] = {
                'class': 'VLAN',
                'mtu': link['mtu'],
                'tag': link['segmentation_id'],
                'interfaces': [{'name': link['interface'], 'tagged': link['tagged']}]
            }
        # f5-declarative-onboarding for L3 interfaces
        for n_link in selfips:
            selfip = selfips[n_link]
            if len(selfip['netmask']) > 3:
                if tmos_onboard_utils.is_v4(selfip['ip_address'].split('%')[0]):
                    selfip['netmask'] = tmos_onboard_utils.ipv4_cidr_from_netmask(
                        selfip['netmask'])
                else:
                    selfip['netmask'] = tmos_onboard_utils.ipv6_cidr_from_netmask(
                        selfip['netmask'])
            do_declaration['Common'][selfip['selfip_name']] = {
                'class': 'SelfIp',
                'address': "%s/%s" % (selfip['ip_address'].split('%')[0], selfip['netmask']),
                'vlan': selfip['net_name'],
                'allowService': 'all',
                'trafficGroup': 'traffic-group-local-only'
            }
            ...
            for route in routes[n_link]:
                if not (route['exclude'] or route['use_rd']):
                    if len(route['netmask']) > 3:
                        if tmos_onboard_utils.is_v4(route['gateway'].split('%')[0]):
                            route['netmask'] = tmos_onboard_utils.ipv4_cidr_from_netmask(
                                route['netmask'])
                    else:
                        route['netmask'] = tmos_onboard_utils.ipv6_cidr_from_netmask(
                            route['netmask'])
                    do_declaration['Common'][route['route_name']] = {
                        'class': 'Route',
                        'gw': route['gateway'],
                        'network': "%s/%s" % (route['network'], route['netmask']),
                        'mtu': 0
                    }
                    if route['network'] == '0.0.0.0' or route['network'] == '::':
                        do_declaration[
                            'Common'][route['route_name']]['network'] = 'default'
        ...
        for link in links:
            if links[link]['interface'] == configsync_interface:
                if link in selfips:
                    do_declaration['Common']['configsync'] = {
                        'class': 'ConfigSync',
                        'configsyncIp': selfips[link]['ip_address'].split('%')[0]
                    }
    if forced_tmm_down:
        tmos_onboard_utils.start_tmm()
    return {'onenic': onenic,
            'mgmt_link_id': m_l_id,
            'mgmt_address': m_ip,
            'mgmt_netmask': m_nm,
            'mgmt_gateway': m_gw,
...
def create_mgmt_init_script(resources):
    ...
    log_progress('writing out pre declaration mgmt scripts')
    log_progress('mmgt ip %s/%s' %
                 (resources['mgmt_address'], resources['mgmt_netmask']))
    if resources['mgmt_gateway']:
        log_progress('mmgt gw ' + resources['mgmt_gateway'])
    tmsh_cmd_dir_exists()
...
def create_tmm_net_init_script(resources):
    ...
    log_progress('writing out non declarative network provisioning scripts')
...
def creat_services_init_script(resources):
    ...
    log_progress('writing out services initialization script')
    services_onboard_script = NON_DO_TMSH_CMD_FILE_DIR + '/002_services_setup.sh'
...
def create_cmi_init_script(resources):
    ...
    log_progress('writing out CMI initialization script')
            ...
            for link in resources['links']:
                if resources['links'][link]['interface'] == resources['configsync_interface']:
                    if link in resources['selfips']:
                        ...
                        log_progress('configsync configuration incomplete')
            cmi_script.write(
    ...
    if post_onboard_commands:
        ...
        with open(post_onboard_commands_script, 'w') as pob_script:
            ...
            for cmd in post_onboard_commands:
                pob_script.write("\"%s\"" % cmd.replace('"', r'\"'))
            pob_script.write(")\n\n")
...
def create_onboard_artifacts(resources, do_enabled=True,
    ...
    if do_enabled and 'do_declaration' in resources:
    ...
    if as3_declaration:
        try:
            ...
            log_progress('exception persisting AS3 declaration: %s' % err)


def create_pre_do_onboard():
    """Creates the initialization script for all pre f5-declarative-onboarding tasks"""
    log_progress('writing out pre declaration onboard initialization script')
...
def create_non_do_onboard():
    ...
    log_progress('writing out non declared onboard initialization script')
...
def onboard(do_enabled, as3_enabled, post_onboard_enabled):
    ...
    with open('/var/log/onboard.log', 'a+') as onboardlog:
        log_progress('running pre declared onboarding scripts')
        subprocess.call(['nohup', 'sh', '-c', pre_do_onboard_script,
                         '&'], stdout=onboardlog, stderr=onboardlog)
    wait_for_pre_do_onboarding = 120
    while wait_for_pre_do_onboarding > 0:
        wait_for_pre_do_onboarding -= 1
        if not os.path.exists(PRE_DO_ONBOARD_COMPLETE_FLAG_FILE):
            time.sleep(1)
        else:
            wait_for_pre_do_onboarding = 0
    ...
    if non_do_onboard:
        ...
        with open('/var/log/onboard.log', 'a+') as onboardlog:
            log_progress('running non declarative onboarding script')
            subprocess.call(['nohup', 'sh', '-c', non_do_onboard_script,
                             '&'], stdout=onboardlog, stderr=onboardlog)
        wait_for_non_do_onboarding = 120
        while wait_for_non_do_onboarding > 0:
            wait_for_non_do_onboarding -= 1
            if not os.path.exists(NON_DO_ONBOARD_COMPLETE_FLAG_FILE):
                time.sleep(1)
            else:
                wait_for_non_do_onboarding = 0
    ...
    if post_onboard_enabled:
        ...
        with open('/var/log/onboard.log', 'a+') as onboardlog:
            log_progress('running post onboard script')
            subprocess.call(['nohup', 'sh', '-c', post_onboard_script,
                             '&'], stdout=onboardlog, stderr=onboardlog)
        wait_for_post_onboarding = 120
        while wait_for_post_onboarding > 0:
            wait_for_post_onboarding -= 1
            if not os.path.exists(POST_ONBOARD_FLAG_FILE):
                time.sleep(1)
            else:
                wait_for_post_onboarding = 0
...
def handle(name, cloud_config, cloud, log, args):
    ...
    tag = 'tmos_configdrive_openstack'
    enabled = False
    ...
    if enabled:
        ...
        if 'dhcp_timeout' in cloud_config[tag]:
            dhcp_timeout = int(
                cloud_config[tag]['dhcp_timeout'])
        if 'inject_routes' in cloud_config[tag]:
            inject_routes = bool(
                cloud_config[tag]['inject_routes'])
        icontrollx_package_urls = []
        if 'icontrollx_package_urls' in cloud_config[tag]:
            icontrollx_package_urls = cloud_config[tag]['icontrol_package_urls']
        do_enabled = True
        if 'do_enabled' in cloud_config[tag]:
            do_enabled = bool(
                cloud_config[tag]['do_enabled'])
        do_additional_declarations = None
        ...
        resources = resolve_resources(rd_enabled,
                                      ...
                                      dhcp_timeout)
        log_progress(
            'resolved config resources: %s' % resources)
        ...
        try:
            onboard(do_enabled, as3_enabled, post_onboard_enabled)
        except Exception as err:
...
if __name__ == "__main__":
    ...
    handle('cc_tmos_openstack_network_data_config',
