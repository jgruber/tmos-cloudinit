                ...
                if not (route['exclude'] or route['use_rd']):
                    if len(route['netmask']) > 3:
                        if tmos_onboard_utils.is_v4(route['gateway'].split('%')[0]):
                            route['netmask'] = tmos_onboard_utils.ipv4_cidr_from_netmask(route['netmask'])
                    else:
                    ...
                    if route['network'] == '0.0.0.0' or route['network'] == '::':
                        do_declaration['Common'][route['route_name']]['network'] = 'default'
            ...
            for link in resources['links']:
                if resources['links'][link]['interface'] == resources['configsync_interface']:
                    if link in resources['selfips']:
                        ...
                        cmi_script.write("tmsh modify cm device %s configsync-ip %s unicast-address { { effective-ip %s effective-port 1026 ip %s } } mirror-ip %s\n" % (
...
def onboard(do_enabled, as3_enabled):
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(do_enabled, as3_enabled)
        except Exception as err:
