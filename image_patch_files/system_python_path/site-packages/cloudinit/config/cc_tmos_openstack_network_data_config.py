...
LOG = logging.getLogger('tmos_configdrive_openstack')
...
def resolve_resources(rd_enabled=True, default_route_interface=None,
    ...
    if default_route_interface:
        LOG.debug('default_route_interface %s requested', default_route_interface)
    else:
        LOG.debug(
        ...
        if link['type'] == 'vlan':
            if link['vlan_link'] not in links:
                ...
                if not onenic and links[link['vlan_link']]['interface_index'] == 0:
                    LOG.warn("VLAN tagging is not supported on management interface")
                else:
        ...
        if net['type'] == 'ipv4_dhcp':
            if net['link'] == m_l_id:
                try:
                    ...
                    LOG.error('exception in processing mgmt DHCP lease file: %s', err)
            else:
                    ...
                    if tmos_onboard_utils.make_dhcp4_request(interface, dhcp_timeout):
                        ...
                        if s_gw and s_gw != m_gw:
                            ...
                            if configsync_interface == links[net['link']]['interface']:
                                LOG.debug(
                                    'configsync_interface discovered with DHCPv4 as link: %s', net['link'])
                                route['use_rd'] = False
                        ...
                        if default_route_interface == links[net['link']]['interface']:
                            LOG.warn('the requested default_route_interface self IP could not be resolved')
                        if configsync_interface == links[net['link']]['interface']:
                            LOG.warn('the requested configsync_interface self IP could not be resolved')
                except Exception as err:
                    LOG.error('exception processing DHCPv4 for interface %s - %s', interface, err)
                            ...
                            if route['network'] == '0.0.0.0' or route['network'] == '::':
                                if route['gateway'] != m_gw:
                                    route['route_name'] = net['link'] + \
                                        '_default_gw'
                                    if default_route_interface == links[net['link']]['interface']:
                                        LOG.debug('default_route_interface discovered as interface: %s gw: %s',
                                                  links[net['link']]['interface'], route['gateway'])
                                        route['use_rd'] = False
                                        number_of_default_routes += 1
                                    if configsync_interface == links[net['link']]['interface']:
                                        LOG.debug('configsync_interface discovered as interface: %s',
                                                  links[net['link']]['interface'])
                                        route['use_rd'] = False
                                        if not default_route_interface == configsync_interface:
                                            inject_route = False
                                    if (not default_route_interface) and (number_of_default_routes == 0 and inject_route):
                                        LOG.debug('dynamically setting default_route_interface as interface: %s gw: %s',
                                                  links[net['link']]['interface'], route['gateway'])
                                        route['use_rd'] = False
                                        number_of_default_routes += 1
...
def create_mgmt_init_script(resources):
    ...
    LOG.debug('mmgt ip %s/%s', resources['mgmt_address'], resources['mgmt_netmask'])
...
def onboard(icontrollx_package_urls, do_enabled, as3_enabled,
            post_onboard_enabled, phone_home_url):
    ...
    if os.path.exists(PRE_DO_ONBOARD_COMPLETE_FLAG_FILE):
        ...
        for ext_url in icontrollx_package_urls:
            ...
            if tmos_onboard_utils.download_extension(ext_url):
                LOG.info('extension %s downloaded in discovered iControl extensions', ext_url)
            else:
        ...
        if do_enabled:
            ...
            tmos_onboard_utils.wait_for_rest_worker('/mgmt/shared/declarative-onboarding', timeout=30)
        ...
        if do_posted or os.path.exists(NON_DO_ONBOARD_COMPLETE_FLAG_FILE):
            if as3_enabled:
                if non_do_onboard:
                    tmos_onboard_utils.wait_for_icontrollx()
                    if tmos_onboard_utils.as3_declare():
                        as3_posted = True
                    else:
                        LOG.error('f5-appsvcs-extension failed to POST a declaration properly')
                    ...
                    if injected_already == '0':
                        LOG.info('injecting f5-appsvcs-extension delayed deployment script %s into /config/startup',
                                 deploy_script_name)
                        with open('/config/startup', 'a+') as css:
                            css.write('/usr/bin/env python %s &' % deploy_script_name)
                    LOG.info('running f5-appsvcs-extension delayed deployment script %s', deploy_script_name)
                    subprocess.call(['/usr/bin/env', 'python', deploy_script_name, '&'])
                    LOG.info('f5-appsvcs-extension declaration delaying for f5-declarative-onboaring to complete. Check log for declaration results.')
                    as3_posted = True
            ...
            LOG.error('neither f5-declarative-onboarding posting or non declarative scripts completed')
            onboard_status = 'ERROR'
    else:
    ...
    if phone_home_url:
        tmos_onboard_utils.phone_home(phone_home_url, do_enabled, as3_enabled, onboard_status)
