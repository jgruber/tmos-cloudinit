    ...
    for net in os_nmd['networks']:
        ...
        if net['type'] == 'ipv4_dhcp':
            if net['link'] == m_l_id:
                ...
                try:
                    if tmos_onboard_utils.make_dhcp4_request(interface, dhcp_timeout):
                        ...
                        if s_gw and s_gw != m_gw:
                            ...
                            if default_route_interface == links[net['link']]['interface']:
                                log_progress(
                                    'default_route_interface discovered with DHCPv4 as interface: %s gw: %s' % (links[net['link']]['interface'], s_gw))
                                route['use_rd'] = False
                                number_of_default_routes += 1
                            if configsync_interface == links[net['link']]['interface']:
                                log_progress(
                                    'configsync_interface discovered with DHCPv4 as link: ' + net['link'])
                                route['use_rd'] = False
                            ...
                            if (not default_route_interface) and (number_of_default_routes == 0 and inject_route):
                                log_progress(
                                    'dynamically setting default_route_interface with DHCPv4 interface: %s gw: %s' % (links[net['link']]['interface'], s_gw))
                                route['use_rd'] = False
                        ...
                        for route in tmos_onboard_utils.process_dhcp4_routes(s_routes):
                            route['use_rd'] = True
                            route['exclude'] = False
                            if default_route_interface == links[net['link']]['interface']:
                                route['use_rd'] = False
...
def create_post_onboard():
    ...
    log_progress('writing out post onboard initialization script')
...
def onboard(icontrollx_package_urls, do_enabled, as3_enabled, post_onboard_enabled):
    ...
    for ext_url in icontrollx_package_urls:
        try:
            log_progress('downloading: %s' % ext_url)
            parsed_url = urlparse.urlparse(extension_url)
            fqdn = parsed_url.netloc
            if(tmos_onboard_utils.wait_for_dns_resolution(fqdn, 120)):
                if tmos_onboard_utils.download_extension(ext_url):
                    log_progress('extension %s downloaded in discoved iControl extensions' % ext_url)
                else:
                    log_progress('exenstion %s could not be donwloaded' % ext_url)
        except Exception as err:
            log_progress("could not download: %s - %s" % (ext_url, err))
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        create_post_onboard()
        try:
            onboard(icontrollx_package_urls, do_enabled,
                    as3_enabled, post_onboard_enabled)
        except Exception as err:
