...
def onboard(do_declaration, as3_declaration, phone_home_url):
    ...
    if as3_declaration:
        try:
            ...
            LOG.error('could not persist f5-appsvcs-extension declaration %s', err)
            as3_enabled = False
    else:
        as3_enabled = False
    ...
    if do_posted and as3_enabled:
        ...
        if injected_already == '0':
            LOG.info('injecting f5-appsvcs-extension delayed deployment script %s into /config/startup', deploy_script_name)
        ...
        as3_posted = True
    elif as3_enabled and not do_enabled:
        ...
        if response:
            ...
            if response.status_code == 204:
                ...
                if not as3_declared:
                    ...
                    LOG.info('f5-appsvcs-extension initial declaration submitted successfully')
                    as3_posted = True
            else:
                LOG.warn(
                    'f5-appsvcs-extension has a previous declaration, not deploying initial declaration')
        else:
            LOG.error(
    ...
    if as3_enabled and not as3_posted:
        onboard_status = 'ERROR'
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(do_declaration, as3_declaration, phone_home_url)
        except Exception as err:
