...
def create_onboard_artifacts(mgmt_cidr, mgmt_gw, mgmt_mtu, post_onboard_commands):
    ...
    with open(network_onboard_script, 'w') as mgmt_script:
        ...
        mgmt_script.write("systemctl disable dhclient\n")
...
def onboard(icontrollx_package_urls, phone_home_url, phone_home_cli):
    ...
    if phone_home_url:
        tmos_onboard_utils.phone_home(
            phone_home_url, False, False, onboard_status)
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(icontrollx_package_urls, phone_home_url, phone_home_cli)
