...
LOG = logging.getLogger('tmos_static_mgmt')
...
def onboard(icontrollx_package_urls, phone_home_url):
    ...
    if phone_home_url:
        tmos_onboard_utils.phone_home(phone_home_url, False, False, onboard_status)
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        if mgmt_ip.find('/') < 0:
            if not mgmt_netmask:
                LOG.error("%s ip is not in CIDR format and no netmask supplied", tag)
                return
        ...
        onboard(icontrollx_package_urls, phone_home_url)
