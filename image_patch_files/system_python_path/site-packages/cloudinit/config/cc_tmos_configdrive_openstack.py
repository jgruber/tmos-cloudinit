...
def create_mgmt_init_script(resources):
    ...
    with open(mgmt_onboard_script, 'w') as mgmt_script:
        ...
        if resources['onenic']:
            ...
            mgmt_script.write("tmsh create net vlan %s mtu %s interfaces replace-all-with { %s } tag %s\n" % (
                'internal', resources['mgmt_mtu'], '1.0', '4094'))
            mgmt_script.write(
                "tmsh create net self self_1nic { address %s/%s allow-service all vlan internal }\n" % (
                    resources['mgmt_address'], resources['mgmt_netmask']))
            if resources['mgmt_gateway']:
                mgmt_script.write(
