...
DO_DELAYED_DEPLOYMENT_SCRIPT = 'cc_tmos_do_only_wait.py'
...
def onboard(trusted_sources, icontrollx_package_urls,
            do_enabled, as3_enabled, post_onboard_enabled,
    ...
    if os.path.exists(PRE_DO_ONBOARD_COMPLETE_FLAG_FILE):
        ...
        if do_enabled:
            ...
            if icontrol_available:
                LOG.debug(
                    'waiting for f5-declarative-onbaording to become available and settle')
                do_available = False
                for _ in range(5):
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(icontrollx_trusted_sources,
                    icontrollx_package_urls, do_enabled,
