...
def wait_for_do_to_complete():
    ...
    while (end_time - time.time()) > 0:
        try:
            ...
            time.sleep(1)
    LOG.error('f5-declarative-onboarding did successfully complete after %s', DO_TIMEOUT)
    return False
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if wait_for_do_to_complete():
        ...
        if response:
            LOG.info('making initial f5-appsrvs-3 declaration')
            if response.status_code == 204:
                as3_declared = tmos_onboard_utils.as3_declare()
                if not as3_declared:
                    LOG.error('f5-appsrvs-3 inital declaration failed')
                else:
                    LOG.info('f5-appsrvs-3 initial declaration submitted successfully')
            else:
                LOG.warn(
                    'f5-appsrvs-3 has a previous declaration, not deploying initial declaration')
        else:
            LOG.error(
                'f5-appsrvs-3 has failed, not deploying initial declaration declaration')
    else:
        LOG.error(
            'f5-declarative-onboarding has failed, not deploying f5-appsrvs-3 initial declaration')
