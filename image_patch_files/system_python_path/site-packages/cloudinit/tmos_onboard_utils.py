...
def validate_rpm_package(rpm_file_path):
    ...
    fnull = open(os.devnull, 'w')
    rpm_check_out = subprocess.Popen(
        "rpm -K %s" % rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def get_rpm_packager(rpm_file_path):
    ...
    return subprocess.Popen(
        "rpm -qp --queryformat \"%{PACKAGER}\" %s" % rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def get_rpm_summary(rpm_file_path):
    fnull = open(os.devnull, 'w')
    return subprocess.Popen(
        "rpm -qp --queryformat \"%{SUMMARY}\" %s" % rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def get_rpm_package_name(rpm_file_path):
    fnull = open(os.devnull, 'w')
    return subprocess.Popen(
        "rpm -qp --queryformat \"%{NAME}\" %s" % rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def install_extensions(trusted_sources=True):
    ...
    for pkg in os.listdir(PKG_INSTALL_DIR):
        if wait_for_mcpd() and wait_for_rest_worker('/mgmt/shared/iapp/package-management-tasks/'):
            ...
            if ext == 'rpm':
                (int_check, sig_validated) = validate_rpm_package(
                    '%s/%s' % (PKG_INSTALL_DIR, pkg))
                if not int_check:
                    LOG.error('%s is not a valid RPM package or is corrupt')
                    continue
                if trusted_sources and not sig_validated:
                    LOG.warning('package %s not signed by F5 Networks.. ignoring', pkg)
                    continue
                LOG.info('Installing: %s - %s',
                         get_rpm_package_name(), get_rpm_summary())
                ...
                if rpm_installed:
                    LOG.debug(
                        'icontrol LX rpm %s installed.. restnoded will restart', pkg)
                    time.sleep(5)
                else:
                    LOG.error(
                        'icontrol LX rpm %s did not install properly', pkg)
                wait_for_icontrollx()
            elif ext == 'fsi':
                ...
                LOG.error(
                    "%s extension: %s, is not a recognized package type", pkg, ext)
    return True
