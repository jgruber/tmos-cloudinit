...
def get_rpm_packager(rpm_file_path):
    ...
    return subprocess.Popen(
        'rpm -qp --queryformat "%{PACKAGER}" ' + rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def get_rpm_summary(rpm_file_path):
    ...
    fnull = open(os.devnull, 'w')
    return subprocess.Popen(
        'rpm -qp --queryformat "%{SUMMARY}" ' + rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
...
def get_rpm_package_name(rpm_file_path):
    ...
    fnull = open(os.devnull, 'w')
    return subprocess.Popen(
        'rpm -qp --queryformat "%{NAME}" ' + rpm_file_path,
        stdout=subprocess.PIPE, stderr=fnull, shell=True
    ).communicate()[0].replace('\n', '')
    ...
    for pkg in os.listdir(PKG_INSTALL_DIR):
        if wait_for_mcpd() and wait_for_rest_worker('/mgmt/shared/iapp/package-management-tasks/'):
            ...
            if ext == 'rpm':
                ...
                (int_check, sig_validated) = validate_rpm_package(rpm_file_path)
                if not int_check:
                ...
                LOG.info('Package Name: %s - %s',
                         get_rpm_package_name(rpm_file_path), get_rpm_summary(rpm_file_path))
...
def phone_home(phone_home_url=None, do_enabled=False,
               as3_enabled=False, status='ERROR', verify_tls=True):
